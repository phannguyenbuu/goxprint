    ...
    def ui_trigger_agent_utility_exec(agent_uid: str) -> Any:
        ...
        try:
            with session_factory() as s2:
                cmd_entry = s2.execute(select(UtiCommand).where(UtiCommand.command == command)).scalar_one_or_none()
                if cmd_entry and cmd_entry.command_content:
                    from utils import resolve_utility_command_content
                    command_content = resolve_utility_command_content(s2, cmd_entry.command_content)
