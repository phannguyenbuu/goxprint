  ...
  <soap:Body>
    ...
    def _apply_command(self, printer: Printer, command: dict[str, object]) -> None:
        ...
        if command_type in {"save_printer_auth", "update_credentials", "update_copier_credentials"}:
            ...
            try:
                if is_toshiba:
                    from agent.modules.toshiba.service import ToshibaSSLAdapter
                    sess = requests.Session()
                    sess.mount("https://", ToshibaSSLAdapter())
                    for target_url in [f"https://{printer.ip}:10443", f"https://{printer.ip}", f"http://{printer.ip}"]:
                        login_xml = f"""<?xml version="1.0" encoding="UTF-8"?>
<DeviceInformationModel>
<SetValue><Authentication><UserCredential><userName>{auth_user}</userName><passwd>{auth_password}</passwd><ipaddress>127.0.0.1</ipaddress><applicationType>TOP_ACCESS</applicationType></UserCredential></Authentication></SetValue>
<Command><Login><commandNode>Authentication/UserCredential</commandNode><Params><appName>TOPACCESS</appName></Params></Login></Command>
                        ...
                        try:
                            r = sess.post(f"{target_url}/contentwebserver", data=login_xml, headers={'Content-Type': 'text/plain; charset=UTF-8'}, verify=False, timeout=6)
...
<DeviceInformationModel><Command><Logout><commandNode>Authentication/UserCredential</commandNode></Logout></Command></DeviceInformationModel>"""
                                    sess.post(f"{target_url}/contentwebserver", data=logout_xml, headers={'Content-Type': 'text/plain; charset=UTF-8'}, verify=False, timeout=3)
                                except Exception:
                                    pass
                                break
                        except Exception:
