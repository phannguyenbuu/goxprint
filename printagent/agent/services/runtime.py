...
def acquire_single_instance(name: str) -> tuple[SingleInstanceLock | None, bool]:
    ...
    for attempt in range(max_retries):
        ...
        if attempt < max_retries - 1:
            # If we acquired the file lock, but Mutex exists, the Mutex is held by a zombie child!
            # Kill any other printagent/goxprint process to release the Mutex.
            import psutil
            try:
                for proc in psutil.process_iter(['name', 'pid']):
                    p_name = (proc.info['name'] or '').lower()
                    p_pid = proc.info['pid']
                    if p_pid != os.getpid() and ('printagent' in p_name or 'goxprint' in p_name):
                        LOGGER.warning("[SingleInstance] Zombie %s (PID %d) holding Mutex. Killing it.", p_name, p_pid)
                        proc.kill()
            except Exception as e:
                LOGGER.warning("[SingleInstance] Error killing zombies: %s", e)
...
def remove_startup_registration(app_name: str = "GoPrinxAgentFtpWorker") -> tuple[bool, str]:
    if not is_windows():
