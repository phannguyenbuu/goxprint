...
WATCHDOG_BAT_CONTENT = (
    ...
    '    echo [Watchdog] Found update file for printagent. Applying...\n'
...
def kill_existing_processes():
    ...
    print("Cleaning up old update files to prevent update loops...")
    try:
        import glob
    ...
    try:
        ...
        subprocess.run(["wmic", "process", "where", "name='cmd.exe' and commandline like '%watchdog.bat%'", "call", "terminate"], capture_output=True, creationflags=0x08000000)
        subprocess.run(["wmic", "process", "where", "name='wscript.exe' and commandline like '%run_watchdog.vbs%'", "call", "terminate"], capture_output=True, creationflags=0x08000000)
    except Exception:
        pass
    time.sleep(1.5)
    ...
    # Clean up old update_notice.json to avoid version stuck issue
    try:
        import tempfile
        for notice_path in [
            INSTALL_DIR / "storage" / "data" / "update_notice.json",
...
def download_binary(dest_path: Path, api_url: str, binary_name: str) -> bool:
    print(f"Đang tải {binary_name} mới nhất...")
    base_url = api_url.strip().rstrip("/")
    if base_url.endswith("/api"):
        base_url = base_url[:-4]
    
    download_url = f"{base_url}/static/releases/{binary_name}"
    ...
    candidate_urls = [
        ...
        f"http://agentapi.quanlymay.com/static/releases/{binary_name}?{cache_buster}",
    ]
...
def main():
    ...
    success1 = download_binary(dest_exe, api_url, 'printagent.exe')
    ftp_exe = INSTALL_DIR / "gox_ftp_server.exe"
    success2 = download_binary(ftp_exe, api_url, "gox_ftp_server.exe")
