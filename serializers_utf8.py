...
from models import (
    ...
    Task, UserAccount, NetworkInfo, Workspace, UserWorkspace, Location, RepairRequest, Material, Lead, IPData
)
from utils import (
...
def _set_printer_online_state(session: Any, printer: Printer, is_online: bool, changed_at: datetime) -> None:
    ...
    # 1. Chronological check: Drop delayed packets
    if printer.online_changed_at and changed_at < printer.online_changed_at:
        return
        
    if next_state is True:
        # 2. Offline Wins check: If any other agent reported offline recently, reject this Online update
        ...
        if printer.mac_address:
            recent_offline = session.execute(
                select(DeviceInfor).where(
                    DeviceInfor.lead == printer.lead,
                    func.upper(DeviceInfor.mac_id) == func.upper(printer.mac_address),
                    DeviceInfor.last_status_at >= recent_cutoff
                )
            ).scalars().all()
            
            for dev in recent_offline:
                if isinstance(dev.status_data, dict) and dev.status_data.get("is_online") is False:
                    # Found a recent offline report, reject the online transition
                    return
