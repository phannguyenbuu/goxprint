...
def register_public_core_routes(app: Flask, session_factory: Any, lead_key_map: dict[str, str]) -> None:
    ...
    def public_device_by_mac_cached() -> Any:
        mac_input = _to_text(request.args.get("mac_id") or request.args.get("mac"))
        if not mac_input:
            return jsonify({"ok": False, "error": "Missing parameter: mac_id"}), 400
        ...
        with session_factory() as session:
            ...
            agent_uid = (dev_info and dev_info.agent_uid) or (p_obj and p_obj.agent_uid) or (dev_hist and dev_hist.agent_uid) or ""
            printer_name = (dev_info and dev_info.printer_name) or (p_obj and p_obj.printer_name) or (dev_hist and dev_hist.printer_name) or ""
            ip = (dev_info and dev_info.ip) or (p_obj and p_obj.ip) or (dev_hist and dev_hist.ip) or ""
            counter_data = (dev_info and dev_info.counter_data) or (dev_hist and dev_hist.counter_data) or {}
            status_data = (dev_info and dev_info.status_data) or (dev_hist and dev_hist.status_data) or {}

            if mem_device:
                if counter_data and not mem_device.get("counter"):
                    mem_device["counter"] = counter_data
                if status_data and not mem_device.get("status"):
                    mem_device["status"] = status_data
                return jsonify(mem_device)

            if p_obj or dev_info or dev_hist:
                return jsonify({
                    "ok": True,
                    "mac_id": normalized_mac,
                    "lead": (p_obj and p_obj.lead) or "default",
                    "lan_uid": (p_obj and p_obj.lan_uid) or "default",
                    "agent_uid": agent_uid,
                    "printer_name": printer_name,
                    "ip": ip,
                    "counter": counter_data,
                    "status": status_data,
                    "last_seen_at": (dev_info and dev_info.updated_at and dev_info.updated_at.isoformat()) or (p_obj and p_obj.updated_at and p_obj.updated_at.isoformat()) or "",
                })
