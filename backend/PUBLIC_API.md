...
## 17) Save Scan Point Address Book Data
- Method: `POST`
- Path: `/api/scan-points/save`
- Description: Save or completely overwrite the scan point address book data for a given MAC address.
- Request Body (JSON):
  - `mac_id` (or `mac_address`): The MAC address of the device (required).
  - `address_book_data`: The JSON object containing the address list (required).
  - `printer_name` (optional): The name of the copier.
  - `ip` (or `printer_ip`) (optional): The IP address of the copier.
  - `agent_uid` (optional): The agent unique ID.
- Response:
  ```json
  {
    "ok": true,
    "message": "Successfully saved scan point for MAC 58:38:79:79:A3:EB"
  }
  ```
