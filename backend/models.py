...
class LanSite(Base):
    ...
    address: Mapped[str | None] = mapped_column(String(255), default="", nullable=True)
...
class PrinterAuthCredential(Base):
    """Dedicated VPS table storing copier web login credentials (auth_user, auth_password) by MAC address.

    Decouples credential retention from local printers.json files so scans are 100% fresh.
    """
    __tablename__ = "PrinterAuthCredential"
    __table_args__ = (
        UniqueConstraint("lead", "mac_address", name="uq_printerauth_lead_mac"),
    )
    ...
    mac_address: Mapped[str] = mapped_column(String(64), index=True)
...
class PrinterControlCommand(Base):
    ...
    auth_user: Mapped[str] = mapped_column(String(128), default="")
    auth_password: Mapped[str] = mapped_column(String(255), default="")
    driver_brand: Mapped[str] = mapped_column(String(64), default="")
...
class PrinterRecognizePort(Base):
    """VPS table storing printer discovery ports configured for LAN subnet scanning."""
    __tablename__ = "PrinterRecognizePort"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    port: Mapped[int] = mapped_column(Integer, unique=True, index=True)
    port_type: Mapped[str] = mapped_column(String(32), default="definitive", index=True)  # definitive | web | snmp
...
class UtiCommand(Base):
    """VPS table storing utility/exec command templates."""
    __tablename__ = "uti_commands"
...
class IPData(Base):
    __tablename__ = "IPDatas"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    agent_uid: Mapped[str] = mapped_column(String(128), default="")
    lan_uid: Mapped[str] = mapped_column(String(128), default="")
