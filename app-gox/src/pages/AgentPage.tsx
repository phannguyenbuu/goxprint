...
export function AgentPage() {
  ...
  // Agent utilities IP configuration states
  const [adapterName, setAdapterName] = useState('Ethernet');
              ...
              {activeModal === 'utilities' && selectedUtilityAgent && (
                <>
                  ...
                  <div style={{ ...styles.modalBody, gap: '16px', display: 'flex', flexDirection: 'column' }}>
                    ...
                    {/* Section 3: Đổi IP máy PC */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '12px', padding: '16px', borderRadius: '8px', border: '1px solid var(--color-border)', background: 'rgba(255, 255, 255, 0.02)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <h4 style={{ margin: 0, fontSize: '0.85rem', fontWeight: 700, color: 'var(--color-text)', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '6px' }}>
                          🔄 Đổi IP máy PC
                        </h4>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          <label style={{ fontSize: '0.7rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>Tên Adapter mạng (ví dụ: Ethernet, Wi-Fi)</label>
                          <input
                            type="text"
                            value={adapterName}
                        ...
                        <div style={{ display: 'flex', gap: '12px', marginTop: '4px' }}>
                          <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', color: 'var(--color-text)', cursor: 'pointer' }}>
                            <input
                              type="radio"
                              name="ipMode"
                              checked={ipMode === 'dhcp'}
                        ...
                        {ipMode === 'static' && (
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', padding: '10px', background: 'rgba(0, 0, 0, 0.2)', borderRadius: '6px', border: '1px solid rgba(255, 255, 255, 0.05)', marginTop: '4px' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              <label style={{ fontSize: '0.68rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>IP Address *</label>
                              <input
                                type="text"
                                value={staticIp}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              <label style={{ fontSize: '0.68rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>Subnet Mask *</label>
                              <input
                                type="text"
                                value={subnetMask}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              <label style={{ fontSize: '0.68rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>Default Gateway</label>
                              <input
                                type="text"
                                value={gateway}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              <label style={{ fontSize: '0.68rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>DNS Server</label>
                              <input
                                type="text"
                                value={dnsServer}
                        ...
                        {/* Warning Box */}
                        <div style={{ padding: '8px 12px', background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.2)', borderRadius: '6px', fontSize: '0.7rem', color: '#f59e0b', display: 'flex', gap: '6px', alignItems: 'flex-start', marginTop: '4px' }}>
                          <span>⚠️</span>
                          <span><strong>Lưu ý:</strong> Việc thay đổi IP máy tính sẽ làm ngắt kết nối mạng của Agent tạm thời. Agent cần khởi động lại kết nối với IP mới. Hãy đảm bảo bạn nhập đúng thông tin để không bị mất kết nối hoàn toàn.</span>
                        </div>

                        {/* Apply Button */}
                        <button
                          onClick={() => {
                            if (window.confirm(`Bạn có chắc chắn muốn thay đổi IP của PC sang chế độ ${ipMode === 'dhcp' ? 'DHCP' : 'Static (' + staticIp + ')'} không? Lệnh này sẽ làm ngắt kết nối mạng của Agent trong vài giây.`)) {
                              handleTriggerUtility('change_ip', {
                          ...
                          disabled={utilityActionPending !== null || (ipMode === 'static' && !staticIp)}
                          style={{
                            display: 'flex',
                            justifyContent: 'center',
                            alignItems: 'center',
                            gap: '8px',
                            background: (utilityActionPending !== null || (ipMode === 'static' && !staticIp)) ? 'var(--color-surface)' : 'var(--color-primary)',
                            color: '#fff',
                            border: 'none',
                            borderRadius: '4px',
                            padding: '10px 16px',
                            fontSize: '0.8rem',
                            fontWeight: 700,
                            cursor: (utilityActionPending !== null || (ipMode === 'static' && !staticIp)) ? 'not-allowed' : 'pointer',
                            opacity: (utilityActionPending !== null || (ipMode === 'static' && !staticIp)) ? 0.6 : 1,
                            transition: 'all 0.2s',
                            marginTop: '4px'
                          }}
                        >
                          {utilityActionPending === 'change_ip' ? (
                            <>
                              <LoadingSpinner size="sm" />
                              Đang cấu hình lại IP...
                            </>
                          ) : (
                            'Áp dụng cấu hình IP máy PC'
                          )}
                        </button>
                      </div>
                    </div>
