...
def post_wizard_step(session, ip, items, referer=""):
    ...
    # URL-encode the data (NOT multipart) - matches copier's JS: g_objConnectMachine.ajaxSend()
    from urllib.parse import urlencode
    encoded = urlencode(items)
    resp = session.post(url, data=encoded, headers=headers, timeout=10)
...
def add_user_wizard(session, ip, wim_token, email, ftp_port):
    ...
    # Step 0: Load address list page first (required to establish server-side context)
    log("Loading address list page (required before wizard)...")
    ...
    try:
        ...
        log(f"  Address list loaded: HTTP {resp.status_code}, wimToken={wim_token[:8] if wim_token else 'NO'}...")
    except Exception as e:
        log(f"  WARNING: Could not load address list: {e}")

    # Step 1: Open Wizard via adrsGetUserWizard.cgi
    # IMPORTANT: This call resets wimsesid to "--", so we save and restore it
    log("Opening wizard (preserving session cookie)...")
    ...
    try:
        ...
        if new_token:
            wim_token = new_token
        log(f"  Wizard opened: HTTP {resp.status_code}, new wimToken={wim_token[:8] if wim_token else 'NO'}...")
    except Exception as e:
        log(f"  Wizard open failed: {e} (continuing anyway)")
    # Restore wimsesid if it was reset to "--"
    ...
    if (not current_wimsesid or current_wimsesid == "--") and saved_wimsesid and saved_wimsesid != "--":
        ...
        log(f"  Restored wimsesid (was reset to '{current_wimsesid}')")
