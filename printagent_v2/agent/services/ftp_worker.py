...
from agent.utils.firewall import ensure_ftp_firewall_rules
...
class FtpWorker:
    ...
    def _start_site(self, spec: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            LOGGER.exception("FTP worker failed to start site: name=%s error=%s", safe_name, exc)
    ...
    def _sync_once(self) -> None:
        ...
        for running_name in running_names:
            if running_name not in desired_by_name:
                ...
                with self._lock:
                    self._errors.pop(running_name, None)
        ...
        for safe_name, spec in desired_by_name.items():
            ...
            if not start_result.get("ok"):
                err_msg = str(start_result.get("error", "") or "Unknown FTP worker error")
                with self._lock:
                    self._errors[safe_name] = err_msg
                    cur = self._running.get(safe_name)
                    if cur:
                        cur.error = err_msg
                LOGGER.warning("FTP worker failed to start site: name=%s error=%s", safe_name, err_msg)
            else:
                with self._lock:
                    self._errors.pop(safe_name, None)
    ...
    def _snapshot_state(self) -> dict[str, Any]:
        ...
        for name, err_msg in errors.items():
            config = load_config()
            spec = next((s for s in config.get("sites", []) if normalize_site_name(str(s.get("name", "") or "")) == name), None)
            if spec:
                sites.append(
                    {
                        "name": name,
                        "path": str(spec.get("path", "") or ""),
                        "port": normalize_port(spec.get("port"), default=0),
                        "ftp_url": "",
