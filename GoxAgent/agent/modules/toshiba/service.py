...
<Command>
    <Login>
        ...
        # --- Step 4: GetGroupList + GetTemplateList ---
        ...
        # 4a. GetGroupList (Exec Script logic for Public Template Groups)
        try:
            ...
            r_grp = session.post(cgi_url, data=get_groups_xml.encode("utf-8"), headers=plain_headers, verify=False, timeout=6)
            if r_grp.status_code == 200:
                ...
                for g in root.findall(".//Group"):
                    ...
                    if gname and gname.strip() and gname.strip() not in ("Undefined", "Useful Template"):
                        entry_key = f"{gid}-001"
                        if entry_key not in seen_ids:
                            seen_ids.add(entry_key)
                            entries.append({
                                "registration_no": entry_key,
                                "name": gname.strip(),
                                "key_display": gname.strip(),
                                "title_1": gname.strip(),
                                "title_2": gname.strip(),
                                "title_3": "",
                                "auth_user": "",
            ...
            LOGGER.debug("[ToshibaService] GetGroupList parse warning: %s", grp_err)
        ...
        for group in groups:
            ...
            raw_resp_text = ""
            # Use active CGI without looping fallbacks to avoid massive timeouts on empty groups
            try:
                r = session.post(cgi_url, data=get_templates_xml, headers=plain_headers, verify=False, timeout=10)
                ...
                LOGGER.info("[ToshibaService] GetTemplates POST %s group=%s => status=%d len=%d hasTemplate=%s hasSuccess=%s snippet=%s",
                            cgi_url, group, r.status_code, len(r.text), has_template, has_success, snippet)
                if r.status_code == 200:
                    ...
                    debug_content.append({
                        ...
                        "snippet": snippet,
                        "final_url": str(r.url),
                        "redirected": r.url != cgi_url,
                    })
            except Exception as e:
                LOGGER.debug("[ToshibaService] GetTemplates %s group=%s => ERROR: %s", cgi_url, group, e)
                debug_content.append({
                    "group": group,
                    "url": cgi_url,
                    "error": str(e),
            ...
            if raw_resp_text:
                try:
                    ...
                    for match in pattern.finditer(raw_resp_text):
                        ...
                        template_key = f"{gid}-{tid}"
                        is_group_fallback = any(e.get("registration_no") == template_key and e.get("protocol") == "SMB" and e.get("folder") == f"\\\\{printer.ip}\\scan" for e in entries)
                        if template_key in seen_ids and not is_group_fallback:
                            continue
                        if is_group_fallback:
                            entries = [e for e in entries if not (e.get("registration_no") == template_key and e.get("protocol") == "SMB")]
