...
try:
    ...
    def _patched_meta_version(distribution_name: str) -> str:
        try:
            return _orig_meta_version(distribution_name)
        except Exception:
            if str(distribution_name).lower() in ("werkzeug", "flask", "jinja2", "click"):
                return "3.0.0"
            raise
...
def create_app(
    ...
    template_dir = next((path for path in template_candidates if path.exists()), Path(__file__).resolve().parent.parent / "templates")
    ...
    static_dir = next((path for path in static_candidates if path.exists()), Path(__file__).resolve().parent.parent / "static")
    ...
    from agent.webs.web_device import register_device_routes
    from agent.webs.web_scan_ftp import register_scan_ftp_routes
    from agent.webs.web_ui import register_ui_routes
    from agent.webs.web_camera import register_camera_routes
