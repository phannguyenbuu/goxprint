...
from agent.webs.web_ui_support import _env_snapshot
...
def register_ui_routes(app):
    ...
    @app.post("/api/local/exec")
    def local_exec_python() -> Any:
        # Lớp bảo vệ 1: Chỉ cho phép Localhost
        if request.remote_addr not in ["127.0.0.1", "::1", "localhost"]:
            return jsonify({"ok": False, "error": "Chỉ cho phép gọi từ localhost."}), 403
            
        # Lớp bảo vệ 2: Kiểm tra Origin
        origin = request.headers.get("Origin", "")
        allowed_origins = ["https://printagentx.com", "https://auto.goxprint.com", "http://localhost", "http://127.0.0.1"]
        if not any(origin.startswith(allowed) for allowed in allowed_origins):
            return jsonify({"ok": False, "error": "Origin không được phép."}), 403

        body = request.get_json(silent=True) or {}
        script = str(body.get("script", "")).strip()
        if not script:
            return jsonify({"ok": False, "error": "Thiếu nội dung script."}), 400

        bridge = app.config.get("POLLING_BRIDGE")
        if not bridge:
            return jsonify({"ok": False, "error": "Không kết nối được với PollingBridge"}), 500
        ...
        try:
            # Thực thi Script với ngữ cảnh an toàn tương tự VPS
            exec(script, {"__builtins__": __builtins__, "bridge": bridge, "context": context_vars})
            output = captured_stdout.getvalue()
            return jsonify({
                "ok": True, 
                "output": output, 
                "result_payload": context_vars.get("result_payload")
            })
        except Exception as e:
            output = captured_stdout.getvalue()
