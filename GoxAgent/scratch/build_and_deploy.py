...
def main():
    ...
    agent_routes_path = root / "scratch" / "backend" / "agent_routes.py"
    ...
    try:
        ps1_path = Path(__file__).resolve().parent / "build_agent_loader_exe.ps1"
        subprocess.run(["powershell", "-File", str(ps1_path), new_version], cwd=str(root), check=True)
    ...
    exe_dest = root / "scratch" / "backend" / "static" / "releases" / "printagent.exe"
    ...
    if exe_src.exists():
        ...
        manifest_path = root / "scratch" / "backend" / "storage" / "releases" / "agent_release.json"
    ...
    # 4. Deploy to VPS
    deploy_script = root / "scratch" / "deploy_to_vps.py"
