...
local_backend = root_dir / "scratch" / "backend"
...
# Static assets
local_assets = local_backend / "static" / "assets"
for asset_file in local_assets.glob("*"):
    if asset_file.is_file():
        remote_path = f"{remote_backend}/static/assets/{asset_file.name}"
        ensure_remote_parent_dir(remote_path)
        print(f"Uploading {asset_file} to {remote_path}...")
        sftp.put(str(asset_file), remote_path)
...
try:
    import json
    import time
    manifest_path = root_dir / "scratch" / "backend" / "storage" / "releases" / "agent_release.json"
    version_str = "2.0.16"
